...
public class RecordsDatabaseClient extends Application {
	...
	public void initializeSocket(){
		try {
			...
			System.out.println("Client: Unknown host error: \n"+exception);
		} catch (IOException e) {
    ...
    public void requestService() {
        try {
			...
			OutputStreamWriter requestStreamWriter = new OutputStreamWriter(requestStream, "UTF-8");
    ...
    public void reportServiceOutcome() {
        try {
			...
			ObservableList < Node > nodes = thePrimaryStage.getScene().getRoot().getChildrenUnmodifiable();
			TableView<MyTableRecord> tv = (TableView<MyTableRecord>) nodes.getLast();



			ObservableList < MyTableRecord > tmpRecords = tv.getItems();
			tmpRecords.clear();
			while(serviceOutcome.next()){
				MyTableRecord temp = new MyTableRecord();
				temp.setTitle(serviceOutcome.getString("title"));
				System.out.print(serviceOutcome.getString("title")+" | ");
				temp.setGenre(serviceOutcome.getString("genre"));
				System.out.print(serviceOutcome.getString("genre")+" | ");
				temp.setRrp(serviceOutcome.getString("rrp"));
				System.out.print(serviceOutcome.getString("rrp")+" | ");
				temp.setLabel(serviceOutcome.getString("label"));
				System.out.print(serviceOutcome.getString("label")+" | ");
				temp.setCopyID(serviceOutcome.getString("copyid"));
				System.out.println(serviceOutcome.getString("copyid"));
				tmpRecords.add(temp);

			}
			tv.setItems(tmpRecords);
